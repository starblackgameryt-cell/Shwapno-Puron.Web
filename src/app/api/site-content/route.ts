...
// Default content - used when no custom content is set
...
// GET - Get all site content (public)
export async function GET() {
  try {
    const contents = await db.siteContent.findMany()

    // Merge with defaults
    const result: Record<string, string> = { ...defaultContent }
...
export async function PUT(request: Request) {
  try {
    ...
    if (!data || typeof data !== 'object') {
      return Response.json({ error: 'অবৈধ ডেটা' }, { status: 400 })
    }
    ...
    return Response.json({ error: 'কন্টেন্ট আপডেট করতে সমস্যা হয়েছে' }, { status: 500 })
  }
}
