      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ScrollReveal>
          <div className="text-center mb-10 sm:mb-16">
            <span className="text-[10px] sm:text-[11px] tracking-[0.2em] sm:tracking-[0.3em] uppercase text-brand-gold font-medium font-serif-lux">{get('showcase_label')}</span>
            <h2 className="mt-2 text-2xl sm:text-4xl md:text-5xl font-black text-brand-emerald tracking-tight font-display zari-underline">{get('showcase_title')}</h2>
          </div>
        </ScrollReveal>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-5 lg:gap-6 mb-12 sm:mb-20">
          {features.map((feature, index) => (
            <ScrollReveal key={feature.title} delay={index * 0.1}>
              <div className="bg-brand-ivory p-5 sm:p-8 lg:p-10 text-center silk-hover border border-[#E6DFD0] group">
                <div className="w-12 h-12 sm:w-14 sm:h-14 mx-auto rounded-full bg-brand-ivory flex items-center justify-center mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300">
        ...
        <ScrollReveal>
          <div className="text-center mb-8 sm:mb-12">
            <span className="text-[10px] sm:text-[11px] tracking-[0.2em] sm:tracking-[0.3em] uppercase text-brand-gold font-medium font-serif-lux">গ্যালারি</span>
            <h2 className="mt-2 text-2xl sm:text-4xl md:text-5xl font-black text-brand-emerald tracking-tight font-display zari-underline">শোকেস</h2>
          </div>
        </ScrollReveal>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-4 lg:gap-6">
          {showcaseItems.map((item, index) => (
            <ScrollReveal key={item.src} delay={index * 0.1}>
              <div className={`group relative ${index === 0 ? 'aspect-[3/4] md:row-span-2' : 'aspect-square'} overflow-hidden bg-brand-ivory cursor-pointer`}>
                <Image
                  src={item.src}
                  alt={item.alt}
                  fill
